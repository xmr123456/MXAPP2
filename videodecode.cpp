...
void VideoDecode::seek(int value)
{
    if(m_formatContext){
        int64_t min_ts = value * m_frameRate;
        int64_t ts = value * m_frameRate;
        int64_t max_ts = value * m_frameRate;

        avformat_seek_file(m_formatContext, -1, min_ts, ts, max_ts, AVSEEK_FLAG_ANY);
    }
}
...
const qint64 &VideoDecode::duration()
{
    return m_totalTime;
}
